...
class Logger:
    ...
    dictClasses = {
        ...
        FAILED: (
            "FAILED",
            "Endanwender",
            "Der Sonsor konnte nicht abgeglichen werden.",
        ),
        INFO: ("INFO", "Endanwender", "Eine Mitteilung"),
        ERROR: (
            "ERROR",
            "Betreuer des Messplatzes",
            "Fehler bei der Kommunikation mit der Peripherie",
        ),
        TRACE: ("TRACE", "Betreuer des Messplatzes", "Hilfreiche Zusatzinformation"),
        DEBUG: (
            "DEBUG",
            "Softwareentwickler",
            "Fuer die Fehlersuche bestimmte Information",
        ),
    ...
    def __init__(
        self,
        strFilenameLog,
        dictValues={"title": "Title", "html": "<h1>Title</h1>"},
        iRefresh=0,
    ):
        ...
        if iRefresh > 0:
            dictValues["refresh"] = (
                '<meta http-equiv="refresh" content="%d">' % iRefresh
            )
        else:
            dictValues["refresh"] = ""
        strHeader = (
    ...
    def log(self, strClass, strInfo, strLink=None, strTag="p"):
        ...
        if strLink == None:
            ...
            strInfo = '<a class="%s" href="%s">%s</a>' % (
                strClass,
                strLink,
                html.escape(strInfo),
            )
        for objFile, strRefLink, listExclude in (
            (self.debugFile, self.strLogFile, ()),
            (self.logFile, self.strDebugFile, (TRACE, DEBUG)),
        ):
            if strClass not in listExclude:
                dictValues = {
                    "tag": strTag,
                    "class": strClass,
                    "ref": self.iRef,
                ...
                strLine = (
                    '<%(tag)s class="%(class)s"><a name="ref%(ref)d" class="SMALLLINK" href="%(reflink)s#ref%(ref)d">-&nbsp;&nbsp;</a>%(info)s</%(tag)s>\n'
                    % dictValues
                )
                try:
    ...
    def table(self, strClass, strTitle, listHeader, listRows):
        ...
        self.write(
            '<table border="1" cellpadding="3" cellspacing="0" bordercolordark="white" bordercolorlight="gray">\n',
            strClass,
        )
        self.write("<tr>\n", strClass)
        ...
        for listRow in listRows:
            ...
            for listColumn in listRow:
                ...
                if listText[1] == "":
                    ...
                    listText[1] = "-"
                self.write(
                    "<td>%s</td>\n" % (self.__get_text(listText[1:], listText[0])),
                    strClass,
                )
            self.write("</tr>\n", strClass)
        self.write("</table>\n", strClass)
...
def test():
    objLogger = Logger(
        ...
        {
            "title": "Logger - Test",
            "html": "<h1>Logger Test</h1><p>K. O. R. was here</p><hr>",
        },
    )
    ...
    for iClass in Logger.listClasses:
        ...
        listKlassifizierung.append(
            ((iClass, strClass), (INFO, strBetrifft), (INFO, strComment))
        )
    objLogger.table(
        OK,
        "Klassifizierung der Meldungen",
        ("Klasse", "Die Meldung betrifft", "Bemerkungen"),
        listKlassifizierung,
    )
    ...
    objLogger.table(
        OK,
        "Tabelle mit Links",
        ("Sensorname", "Kalibration", "Kommentar"),
        (
            (
                (INFO, "33a", "http://www.python.org"),
                (FAILED, "Fehler", "http://www.python.org"),
                (FAILED, "Ausserhalb Toleranz"),
            ),
        ),
    )
    ...
    objLogger.log_image(
        "INFO", "logger_test.gif", "http://www.python.org", "Dies ist der Alt-Text"
    )
    ...
    for iClass in Logger.listClasses:
        (strClass, strBetrifft, strComment) = Logger.dictClasses[iClass]
        objLogger.log(
            strClass, "Logeintrag mit Link (%s)" % strClass, "http://www.python.org"
        )
    ...
    try:
        ...
        objLogger.log(
            FATAL,
            "Es wurde absichtlich auf ein nicht existierendes Element zugegriffen",
        )
    try:
...
config = {
    ...
    "regexp_nocompress": [
        r"\.png$",  # '.png' will not be compressed
        r"\.gif$",  # '.gif' will not be compressed
        r"\.jpg$",  # '.jpg' will not be compressed
        r"\.zip$",  # '.zip' will not be compressed
    ],
}
...
class ZipFile:
    ...
    def open(self):
        ...
        self.objZip = zipfile.ZipFile(
            self.strZipFilename, "w", compression=zipfile.ZIP_DEFLATED, allowZip64=True
        )
        self.objBackup.objLogger.log(
            TRACE, os.path.basename(self.strZipFilename), self.strZipFilename
        )
        self.iFiles = 0
        self.iBytes = 0
    ...
    def write(self, strFilename, strAchiveFilename):
      ...
      """
            ...
            self.objBackup.objSummaryLogger.exception(
                LogException(
                    ERROR, "%s: Nicht archiviert! (%s)" % (strAchiveFilename, e)
                )
            )
            return None
            ...
            self.objBackup.objSummaryLogger.exception(
                LogException(
                    FATAL, "%s: Nicht archiviert! (%s)" % (strAchiveFilename, e)
                )
            )
            return None
        objZipInfo = self.objZip.infolist()[-1]
...
class backup:
    def __init__(self, objSummaryLogger, dictBackup):
        ...
        self.dictBackup["backup_filename"] = time.strftime(
            self.dictBackup["backup_filename"], timeNow
        )
        self.dictBackup["backup_directory"] = time.strftime(
            self.dictBackup["backup_directory"], timeNow
        )

        # Build Filenames
        strZipFilenameBase = os.path.abspath(
            "%(backup_directory)s/%(backup_filename)s" % self.dictBackup
        )
        ...
        self.objLogger = Logger(
            strFilenameLog,
            {
                "title": "%(backup_filename)s" % dictBackup,
                "html": "<h1>%(backup_filename)s</h1>" % dictBackup,
            },
            5,
        )
        ...
        self.listRegexpInclude = list(
            map(
                lambda strRegexp: re.compile(strRegexp, re.IGNORECASE),
                self.dictBackup["regexp_include"],
            )
        )
        self.listRegexpExclude = list(
            map(
                lambda strRegexp: re.compile(strRegexp, re.IGNORECASE),
                self.dictBackup["regexp_exclude"],
            )
        )
        self.listRegexpNocompress = list(
            map(
                lambda strRegexp: re.compile(strRegexp, re.IGNORECASE),
                self.dictBackup["regexp_nocompress"],
            )
        )

        self.objSummaryLogger.log(
            INFO, self.dictBackup["backup_filename"], self.objLogger.strLogFile, "h2"
        )
    ...
    def close(self):
        ...
        strCounters = (
            "%(count)d files, %(strUncompressed)s -> %(strCompressed)s bytes"
            % self.dictTotalCounters
        )
        self.objLogger.log(strStatus, strCounters)
        self.objSummaryLogger.log(
            strStatus, "%s: %s" % (self.dictBackup["backup_filename"], strCounters)
        )
    ...
    def funcVisit(self, strSourceBaseDirectory, strDirectory, listNames):
        ...
        strSkipFilenameFull = os.path.abspath(
            os.path.join(strDirectory, strSkipFilename)
        )
        if os.path.exists(strSkipFilenameFull):
            self.objLogger.log(
                INFO, '%s: "%s" found: SKIPPED' % (strDirectory, strSkipFilename)
            )
            return
        ...
        for strFilename in listNames:
            ...
            if os.path.isfile(strFilenameFull):
                if not strFilenameFull.startswith(strSourceBaseDirectory):
                    self.objLogger.log(
                        FATAL,
                        "internal programming error (if not strFilenameFull.startswith(strSourceBaseDirectory):)",
                    )
                ...
                if os.path.getsize(strFilenameFull) > self.dictBackup["max_size"]:
                    self.objLogger.log(
                        ERROR,
                        '%s: File is bigger then "max_size". Skipped!'
                        % strFilenameFull,
                    )
                    continue
                ...
                if objZipInfo == None:
                    ...
                    continue
                # Increment the counters
                for strKey, iValue in (
                    ("count", 1),
                    ("compressed", objZipInfo.compress_size),
                    ("uncompressed", objZipInfo.file_size),
                ):
                    self.dictCounters[strKey] = self.dictCounters[strKey] + iValue
                self.dictCounters["strCompressed"] = add_mille_sep(
                    self.dictCounters["compressed"]
                )
                self.dictCounters["strUncompressed"] = add_mille_sep(
                    self.dictCounters["uncompressed"]
                )
    ...
    def go(self):
        ...
        for strSourceDirectoryRelative in self.dictBackup["src_directories"]:
            ...
            strSourceDirectory = os.path.abspath(
                os.path.join(strSourceBaseDirectory, strSourceDirectoryRelative)
            )
            self.objLogger.log(
                INFO,
                "backing up directory: %s" % strSourceDirectoryRelative,
                strSourceDirectory,
                "h3",
            )
            # os.walk(strSourceDirectory, self.funcVisit, strSourceBaseDirectory)
            for strRootDirectory, listDirectories, listFiles in os.walk(
                strSourceDirectory
            ):
            ...
            if self.dictCounters["count"] == 0:
                self.objLogger.log(
                    ERROR, "Directory is empty! Is the directory name misspelled?"
                )
            # Increment the counters
            for strKey, iValue in self.dictCounters.items():
                self.dictTotalCounters[strKey] = self.dictTotalCounters[strKey] + iValue
            self.dictTotalCounters["strCompressed"] = add_mille_sep(
                self.dictTotalCounters["compressed"]
            )
            self.dictTotalCounters["strUncompressed"] = add_mille_sep(
                self.dictTotalCounters["uncompressed"]
            )
...
def go():
    ...
    # Create Logfile
    strFilenameSummary = os.path.abspath(
        "%(backup_directory)s/%(summary_filename)s.html" % config["default_backup"]
    )
    ...
    objSummaryLogger = Logger(
        strFilenameSummary,
        {"title": os.path.basename(strFilenameSummary), "html": "<h1>Summary</h1>"},
        5,
    )
