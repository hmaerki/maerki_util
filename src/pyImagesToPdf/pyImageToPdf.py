...
import os, re, sys, copy, types, shutil, tempfile, subprocess
...
import reportlab.platypus.figures
...
def getSampleStyleSheet():
    ...
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='Normal',
                                  fontName='Helvetica',
                                  fontSize=iFONT_SIZE_NORMAL,
                                  leading=10)
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='BodyText',
                                  parent=stylesheet['Normal'],
                                  #leftIndent=6,
                                  #firstLineIndent=-6,
                                  spaceBefore=6)
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='BodyTextRight',
                                  parent=stylesheet['BodyText'],
                                  alignment=reportlab.lib.enums.TA_RIGHT)
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='Italic',
                                  parent=stylesheet['BodyText'],
                                  fontName='Helvetica-Italic')
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='h1',
                                  parent=stylesheet['Normal'],
    ...
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='h2',
                                  parent=stylesheet['Normal'],
                                  fontName = 'Helvetica-Bold',
                                  fontSize=iFONT_SIZE_H2,
                                  ),
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='h2Right',
                                  parent=stylesheet['h2'],
                                  alignment=reportlab.lib.enums.TA_RIGHT),
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='h3',
                                  parent=stylesheet['Normal'],
                                  fontName = 'Helvetica-BoldOblique',
                                  fontSize=iFONT_SIZE_H3,
                                  ),
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='h3Right',
                                  parent=stylesheet['h3'],
                                  alignment=reportlab.lib.enums.TA_RIGHT),
                   )
    stylesheet.add(reportlab.lib.styles.ParagraphStyle(name='Bullet',
                                  parent=stylesheet['Normal'],
                                  firstLineIndent=0,
                                  spaceBefore=3),
                  )
...
class PdfRenderer:
  ...
  def __myFirstPage(self, canvas, doc):
    '''
      Header.
    '''
    strFilenameFull, strHeaderLinks, strHeaderMitte, strHeaderRechts, strBookmark = self.listFiles[doc.page-1]
    ...
    canvas.rect(
        0.0, 0.0, doc.width+fMARGINLEFT+fMARGINRIGHT, doc.height+fMARGINTOP+fMARGINBOTTOM,
        stroke=0, fill=1
    )
    ...
    canvas.line(fMARGIN_HEADER_LEFT,yHeader-fMARGIN_HEADER_TOP_LINE, doc.width-fMARGIN_HEADER_RIGHT,yHeader-fMARGIN_HEADER_TOP_LINE)

    # canvas.setFont(strFONT_BOLD, iFONT_SIZE_H2)
    # canvas.setFillColor(reportlab.lib.colors.lightgrey)
    # canvas.setFillColor(reportlab.lib.colors.darkgray)
    canvas.setFont(strFONT_BOLD, iFONT_SIZE_H2)
    canvas.drawRightString(doc.width-fMARGIN_HEADER_RIGHT, yHeader, strHeaderRechts)

    canvas.setFont(strFONT, iFONT_SIZE_H2)
    canvas.drawString(fMARGIN_HEADER_LEFT, yHeader, strHeaderLinks)
    canvas.drawCentredString(fMARGIN_HEADER_LEFT+doc.width/2.0, yHeader, strHeaderMitte)

    canvas.setFont(strFONT, iFONT_SIZE_H4)
    canvas.drawString(fMARGIN_HEADER_LEFT, fMARGIN_FOOTER_BOTTOM, strFilename)
    canvas.drawCentredString(fMARGIN_HEADER_LEFT+doc.width/2.0, fMARGIN_FOOTER_BOTTOM, '%d (%d)' % (doc.page, len(self.listFiles)))

    canvas.restoreState()

  def __startPdfDocument(self, strFilename, strTitle):
    doc = reportlab.platypus.SimpleDocTemplate(strFilename,
  ...
  def getImage(self, strFilenameFull, maxImageWidth, maxImageHeight):
    ...
    return reportlab.platypus.Image(strFilenameFull2, width=width, height=height), width, height
  
  def __dumpPages(self, doc, story):

    for strFilenameFull, strHeaderLinks, strHeaderMitte, strHeaderRechts, strBookmark in self.listFiles:
      strFilename = os.path.basename(strFilenameFull)
      ...
      image, width, height = self.getImage(strFilenameFull, maxImageWidth, maxImageHeight)
  ...
  def createPages(self):
    ...
    doc.build(story, onFirstPage=self.__myFirstPage, onLaterPages=self.__myFirstPage)
...
def convertPdf2Jpg(strFilenameFullWithUnicode):
  ...
  listArgs = [strGsExe, '-dNOPROMPT', '-dNOPAUSE', '-dBATCH', '-sDEVICE=png16m', '-r360', '-sOutputFile='+strFilenameFullPng, strFilenameFull]
  print(' '.join(listArgs))
