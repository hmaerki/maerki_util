...
class http_upload_core:
    def __init__(self):
        ...
        self.objLogger = Logger(
            objMediator.getArgument("FilenameHtmlLog"), objMediator.getArgument("local")
        )
        ...
        self.listRegexpExclude = list(
            map(
                ...
                objMediator.getArgument("exclude", []),
            )
        )
        ...
        if self.objConnection == None:
            if self.strRemoteProtocol == "https":
                ...
                self.objConnection = http.client.HTTPSConnection(
                    self.strRemoteHost, context=sslContext, timeout=30
                )
            else:
                self.objConnection = http.client.HTTPConnection(
                    self.strRemoteHost, timeout=30
                )
    ...
    def http_upload_file(self, strPath):
        ...
        if errorcode in (
            ...
            self.http_create_folder_recursive(strRelativePath)
            errorcode, errormessage = self.http_upload_file_put(
    ...
    def upload_1(self):
        ...
        for round in range(UPLOAD_ROUNDS):
            iErrors = self.upload_2(
                strPath=objMediator.getArgument("local"), round=round
            )
