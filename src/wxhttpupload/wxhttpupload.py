...
from wxhttpupload_wdr import *
import httpupload

# constants

ID_QUIT = 100
strWxHttpUpload = "v1.1.7"
objLogger = None
dictConfig = None
...
class HttpUploadFrame(wx.Frame):
  def __init__(self, parent, id, title,
    ...
    self.dictIcons = {
      'error': wx.Icon(directory + '/resources/wxhttpupload_error.ico', wx.BITMAP_TYPE_ICO),
      'work': wx.Icon(directory + '/resources/wxhttpupload_work.ico', wx.BITMAP_TYPE_ICO),
      'success': wx.Icon(directory + '/resources/wxhttpupload_success.ico', wx.BITMAP_TYPE_ICO),
      'idle': wx.Icon(directory + '/resources/wxhttpupload_idle.ico', wx.BITMAP_TYPE_ICO)
    }
    ...
    self.Bind(wx.EVT_BUTTON, self.OnUploadConfig, self.FindWindowById(ID_BUTTON_UPLOAD_CONFIG))
    self.Bind(wx.EVT_BUTTON, self.OnUploadConfigFolder, self.FindWindowById(ID_BUTTON_UPLOAD_CONFIGFOLDER))
    self.Bind(wx.EVT_BUTTON, self.OnUploadWebdrive, self.FindWindowById(ID_BUTTON_UPLOAD_WEBDRIVE))
    self.Bind(wx.EVT_BUTTON, self.OnUploadGo, self.FindWindowById(ID_BUTTON_UPLOAD_GO))
    self.Bind(wx.EVT_BUTTON, self.OnUploadForce, self.FindWindowById(ID_BUTTON_UPLOAD_FORCE))
    self.Bind(wx.EVT_BUTTON, self.OnUploadErrorlog, self.FindWindowById(ID_BUTTON_UPLOAD_ERRORLOG))
    self.Bind(wx.EVT_CHECKBOX, self.OnUploadAuto, self.FindWindowById(ID_CHECKBOX_AUTOUPLOAD))
    self.Bind(wx.EVT_BUTTON, self.OnUploadLocal, self.FindWindowById(ID_BUTTON_LOCAL))
    self.Bind(wx.EVT_BUTTON, self.OnUploadRemote, self.FindWindowById(ID_BUTTON_REMOTE))
    self.Bind(wx.EVT_BUTTON, self.OnZuluFolder, self.FindWindowById(ID_BUTTON_ZULU_FOLDER))
    self.Bind(wx.EVT_BUTTON, self.OnZuluErrorlog, self.FindWindowById(ID_BUTTON_ZULU_ERRORLOG))
    self.Bind(wx.EVT_BUTTON, self.OnZuluGo, self.FindWindowById(ID_BUTTON_ZULU_GO))
    self.Bind(wx.EVT_BUTTON, self.OnZuluUploadGo, self.FindWindowById(ID_BUTTON_ZULU_UPLOAD_GO))
    self.Bind(wx.EVT_MENU, self.OnQuit, self.FindWindowById(ID_QUIT))
    self.Bind(wx.EVT_CLOSE, self.OnCloseWindow)
    self.Bind(wx.EVT_SIZE, self.OnSize)
    self.Bind(wx.EVT_CHOICE, self.EvtChoice, self.FindWindowById(ID_CHOICE_UPLOAD_CONFIGURATIONS))
  ...
  def setRunning(self, isRunning):
    if isRunning:
      ...
      ctrlCheckBox = objUploadMediator.ctrlFrame.FindWindowById(ID_CHECKBOX_AUTOUPLOAD)
  ...
  def InsertConfigurations(self, listConfigurations):
    ...
    for dictSingleConfig in listConfigurations:
      self.ctrlUploadConfig.Append(dictSingleConfig.get('name', 'Error: name not defined ...'), dictSingleConfig)
  ...
  def UpdateConfiguration(self, bGetValues=False):
    ...
    if bGetValues:
      ...
      dictValues = self.ctrlUploadConfig.GetClientData(self.ctrlUploadConfig.GetSelection())
      for strTagName, iID in dictFields.items():
        dictValues[strTagName] = self.GetValue(iID)
      return dictValues

    dictUploadConfig = self.ctrlUploadConfig.GetClientData(self.ctrlUploadConfig.GetSelection())
    for strTagName, iID in dictFields.items():
      ctrl = self.FindWindowById(iID)
      ctrl.SetValue(dictUploadConfig.get(strTagName, 'Error: Tag "%s" not defined ...' % strTagName))
  ...
  def OnUploadWebdrive(self, event):
    ...
    if self.bWebdriveMount:
      ...
      # Sleep(1000, 0)
      strArgs = '/s:"http_upload_test" /u:"%s" /p:"%s" /url:"%s" /d:%s /pr:1 /exp' % (
  ...
  def OnUploadConfig(self, event):
    win32_ShellExecute('%s\\wxhttpupload_config.txt' % dictConfig['configfile_path'], 'notepad')
  ...
  def OnZuluErrorlog(self, event):
    win32_ShellExecute('%s\\zulu_errorlog.html' % self.GetValue(ID_TEXTCTRL_ZULU_FOLDER))
  ...
  def OnQuit(self, event):
    if objUploadWrapper.running:
      ...
      wx.MessageBox("Stopping the active upload.", "Exit Application", wx.OK, self)
      return
    self.isClosing = True
    self.Close(True)
    self.Destroy()
...
class wxhttpuploadApp(wx.App):
  def OnInit(self):
    ...
    self.frameMain = HttpUploadFrame(None, -1, "wxhttpupload %s" % strWxHttpUpload, wx.Point(20,20), wx.Size(540,500) )
    self.frameMain.Show(True)

    self.InsertConfigurations('%s/wxhttpupload_config.txt' % os.getcwd())
    return True

  def InsertConfigurations(self, strConfigFilename):
    global dictConfig
    dictConfig = {
      'webdrive_drive': 'W:'
    }
    try:
      ...
      objLogger.WriteLine("Configuration File '%s not found in Folder '%s'." % (strConfigFilename, os.getcwd()))
    if dictConfig.get('wxhttpupload_config_version', None) != '1.0.0':
      objLogger.WriteLine("This Configuration-Version is too old for this '%s'" % strConfigFilename)
    
    dictConfig['configfile_path'], dictConfig['configfile_filename'] = os.path.split(strConfigFilename)
    self.frameMain.InsertConfigurations(dictConfig.get('wxhttpupload_configurations', None))
    ...
    while self.keepGoing:
      ...
      if oneShot:
        ...
        if iRetries>iMaxRetries:
          break
        objLogger.WriteLine("Versuch %d von %d folgt in %d Sekunden." % (iRetries, iMaxRetries, iWaitSeconds))
...
class UploadMediator:
  ...
  def setArguments(self, dictArguments):
    ...
    self.dictArguments['FilenameHtmlLog'] = os.path.normpath("%s/tmp_httpupload_log.html" % self.getArgument('local'))
