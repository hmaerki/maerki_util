...
class WorkerThread(threading.Thread):
  ...
  def _scan_page(self):
      ...
      strFilenameFinal = os.path.join(strFolder, self.ctrlApp.ctrlStaticFilenameSimpleScan.GetLabel())
...
class wxPyScanApp(wx.App):
  ...
  def InitFrame(self):
    ...
    self.ctrlChoicePaperSize = wx.xrc.XRCCTRL(self.ctrlFrame, "ID_CHOICE_PAPER_SIZE")
    ...
    self.ctrlStaticFilenameSimpleScan = wx.xrc.XRCCTRL(self.ctrlFrame, "ID_STATIC_FILENAME_SIMPLESCAN")

    self.ctrlButtonMultiScan = wx.xrc.XRCCTRL(self.ctrlFrame, "ID_BUTTON_MULTISCAN")
    self.ctrlFrame.Bind(wx.EVT_BUTTON, self.OnButtonMultiScan, wx.xrc.XRCCTRL(self.ctrlFrame, "ID_BUTTON_MULTISCAN"))
    self.ctrlButtonScan = wx.xrc.XRCCTRL(self.ctrlFrame, "ID_BUTTON_SCAN")
    self.ctrlFrame.Bind(wx.EVT_BUTTON, self.OnButtonScan, wx.xrc.XRCCTRL(self.ctrlFrame, "ID_BUTTON_SCAN"))

    self.ctrlPanelStatus = wx.xrc.XRCCTRL(self.ctrlFrame, "ID_PANEL")

    # wx.EVT_MENU(self.ctrlFrame, wx.xrc.XRCID("wxID_EXIT"), self.OnQuit)
    self.ctrlFrame.Bind(wx.EVT_MENU, self.OnQuit, wx.xrc.XRCCTRL(self.ctrlFrame, "wxID_EXIT"))
    self.ctrlFrame.Bind(wx.EVT_CLOSE, self.OnCloseWindow, wx.xrc.XRCCTRL(self.ctrlFrame, "wxID_CLOSE"))
  ...
  def handleException(self, exception, strMessage=None):
    ...
    if strMessage:
      ...
      self.ctrlErrorMessageDialog = wx.MessageDialog(self.ctrlFrame, strValue, strMessage, wx.OK | wx.ICON_ERROR)
    ...
    self.ctrlErrorMessageDialog = wx.MessageDialog(self.ctrlFrame, 'Programmierfehler...\n %s ' % strValue, 'Fehler', wx.OK | wx.ICON_ERROR)
    self.ctrlErrorMessageDialog.ShowModal()
    self.ctrlErrorMessageDialog = None
  ...
  def OnTimerStatus(self, event):
    try:
      if True:
        ...
        if iQueueSize > 0:
          self.ctrlStatusBar.SetStatusText('Postprocessing: %d images.' % iQueueSize, 0)
        else:
  ...
  def ReplaceFields(self, strFilename):
    ...
    strFilename = strFilename.replace('<<FILEBASE>>', self.ctrlTextFilebase.GetValue())
  ...
  def OnUpdateFilename(self, event=None):
    try:
      for ctrl, strPattern in (
                    (self.ctrlStaticFilenameSimpleScan, self.dictConfig['strPatternSimplescan']),
                  ):
