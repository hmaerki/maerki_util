...
if False:
    ...
    class WiaDeviceManagerSingleton:
        ...
        def __init__(self):
            WiaDeviceManagerSingleton.__wiaDeviceManager = win32com.client.Dispatch(
                WIA_DEVICE
            )
...
def setPropertyById(listProperties, iPropertyId, iValue):
    ...
    listNames = map(lambda p: p.Name, listProperties)
    raise Exception(
        'Property "%d" nicht gefunden %s.' % (iPropertyId, str(listProperties))
    )
...
def getDevice(strScannerDescription):
    ...
    raise Exception(
        'Scanner "%s" nicht gefunden (%s).' % (strScannerDescription, listNames)
    )
...
def setProperties(listProperties, dictValues):
    ...
    if len(listKeys) > 0:
        raise Exception(
            "Properties %s nicht gefunden. Verfuebar sind %s" % (listKeys, listNames)
        )
...
def printProperties(listProperties):
    for wiaProperty in listProperties:
        print(
            "%s: %s (%s)"
            % (wiaProperty.Name, wiaProperty.Value, wiaProperty.IsReadOnly)
        )
...
class scanner:
    ...
    def acquire(self, objVorlage, strPaperSize):
        ...
        dictDeviceItemValues = {
            "Current Intent": iIntentImageType
            + WIA_INTENT_BIAS_MAXIMIZE_QUALITY,  # 4 is Black-white, gray is 2, color 1
        }
        setProperties(self.wiaDeviceItem.Properties, dictDeviceItemValues)
