...
class Zulu:
  ...
  def open(self):
    ...
    if len(sys.argv) > 1:
      try:
        import getopt
        opts, args = getopt.getopt(sys.argv[1:], 'st:', ['structure=', 'template='])
      ...
      if not os.path.exists(self.sFilenameStructure):
        raise ZuluException('File "{}" does not exist!'.format(self.sFilenameStructure))
    ...
    try:
      ...
      excel = zulu.zulu_excel_reader.ExcelReader(self.sFilenameStructure)
    except Exception as e:
      self.objLogger.error('Failed to open the document. (%s)' % (str(e)))
      raise ZuluException()
    if False:
      with open(self.sFilenameStructure.replace('.xlsx', '.dump.txt'), 'w') as obj_file:
        excel.dump(obj_file)
  ...
  def callPython(self, objTemplate, objProcessingState, sText):
    ...
    self.objLogger.error('Error in call to "<!--Zulu:Python:%s-->". This class is not registered.' % sText, objTemplate.strTemplateFilename)
    raise ZuluException()
      ...
      try:
        with open(strInputFilename, 'r', encoding='utf-8') as f:
          try:
            ...
            self.objLogger.error(f"{strInputFilename}: UnicodeDecodeError {err}")
            return
  ...
  def write_output(self, objTemplate, objProcessingState, sStage):
        ...
        if not strOutputFilename in dictOutputFiles:
          ...
          with open(strOutputFilename, 'r', encoding="utf-8") as f:
            try:
              ...
              self.objLogger.error(f"{strOutputFilename}: UnicodeDecodeError {err}")
              text = "?"
    ...
    try:
      ...
      self.objLogger.error('%s: %s. Retry with "replace" instead of "strict"' % (strOutputFilename, e))
  ...
  def zuluility_createFolderRecursive(self, strFilename):
    ...
    if os.path.exists(strPath):
      return
    self.objLogger.info('Creating folder "%s" for file "%s".' % (strPath, strFilename), None)
    os.makedirs(strPath)
...
class HandlerTag:
  ...
  def getValue(self, sTag):
    ...
    try:
      ...
      msg = '"%s": Tag <!--Zulu:Tag:%s--> not found. Used "%s" instead.' % (self.objProcessingState.getIdentification(), sTag, error_tag)
  ...
  def doit(self, objZulu, objTemplate, objProcessingState, sParameter):
    ...
    for objEntry in objNavigation.listEntries:
      ...
      if sOpenAllMenues=='1' or objEntry.to_be_displayed(sPagePath):
        if sCurrentPath==sPagePath or (sSelectParentEntries=='1' and sPagePath.find(sCurrentPath)==0):
    ...
    for objEntry in objNavigation.listEntries:
      ...
      if objEntry.to_be_displayed(sPagePath):
        if (objEntry.iLevel >= iLevelFrom) and (objEntry.iLevel <= iLevelTo):
          if (sCurrentPath == sPagePath) or (sPagePath.find(sCurrentPath) == 0):
            strTemplateName = ':Selected:%s:%d' % (objNavigation.strName, objEntry.iLevel)
          else:
            strTemplateName = ':Normal:%s:%d' % (objNavigation.strName, objEntry.iLevel)
          # Examples ':Normal:Page:0', ':Selected:Page:0'
